...
- CreateSession: Create a new session with the provided endpoint and credentials.
...
- CreateUploader: Create an S3 uploader with the provided session, part size, and concurrency.
...
// CreateUploader creates an S3 uploader with the provided session, part size, and concurrency level
func CreateUploader(sess *session.Session, partSize int64, maxConcurrency int) *s3manager.Uploader {
	// if maxConcurrency > 8 {
	// 	return nil, fmt.Errorf("maxConcurrency should be less than or equal to 8")
	// }
